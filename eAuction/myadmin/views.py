...
def addcategory(request):
    if request.method == "GET":
        ...
        return render(request, "addcategory.html",
                      {'output': 'Category Added Successfully', "sunm": request.session["sunm"]})
...
def addsubcategory(request):
    ...
    if request.method == "GET":
        ...
        return render(request, "addsubcategory.html",
                      {"clist": clist, "output": 'Sub Category Added Successfully', "sunm": request.session["sunm"]})
...
def cpmyadmin(request):
    if request.method == "GET":
        ...
        if res:
            if npass == cnpass:
                myapp_models.Register.objects.filter(username=request.session["sunm"], password=opass).update(
                    password=cnpass)
                return render(request, "cpmyadmin.html", {"sunm": request.session["sunm"],
                                                          "output": "password changed successfully , please login again"})
            else:
                return render(request, "cpmyadmin.html", {"sunm": request.session["sunm"],
                                                          "output": "New & Confirm new password mismatch , try again"})
        else:
            return render(request, "cpmyadmin.html",
                          {"sunm": request.session["sunm"], "output": "Invalid old password , please try again"})
