...
def viewsubcategory(request):
    ...
    return render(request, "viewsubcategory.html",
                  {"catnm": catnm, "clist": clist, "sclist": sclist, "media_url": media_url})
...
def register(request):
    if request.method == "GET":
        ...
        p = models.Register(name=name, username=username, password=password, mobile=mobile, address=address, city=city,
                            gender=gender, role="user", status=0, info=info)
        p.save()
    ...
    if request.method == "GET":
        ...
        if len(userDetails) > 0:
            ...
            return render(request, "login.html",
                          {"output": "Invalid user , please check authentication...", "cunm": cunm, "cpass": cpass})
